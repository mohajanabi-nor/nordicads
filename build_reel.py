"""Nordic Engros – animert REEL (mp4, 9:16). Pop-in, float, glans-sveip, CTA slide."""
...
def build(products, heading, out):
    # statisk base: bg + sirkel + ramme
    ...
    ctaL=Image.new("RGBA",(W,H),(0,0,0,0)); dc=ImageDraw.Draw(ctaL); by=1560
    ...
    ds.text((W//2-ds.textlength("Nye varer på lager hver uke",font=font(600,30))/2,1744),"Nye varer på lager hver uke",font=font(600,30),fill=MUTE)
    # produkter: (bilde, cx, base_y, entrance_start, float_phase, shine?)
    specs=[(products[0],465,1342,10,0.0,False),(products[1],648,1342,22,math.pi,True)]
    frames=[]
    for f in range(N):
        img=base.copy()
        paste_a(img,logoL,(0,0),ease((f-0)/12))
        paste_a(img,headL,(0,0),ease((f-6)/14))
        order=[(0,specs[0]),(1,specs[1])]
        # tegn skygger+produkter bakerst->fremst (flaske bak, boks foran)
        for idx,(prod,cx,base_y,st,ph,sh) in order:
            t=(f-st)/24.0; e=ease(t); a=e
            if a<=0: continue
            entr=(1-e)*210
            fl=7*math.sin(2*math.pi*(f-st)/64.0+ph) if e>=1 else 0
            yoff=entr - fl
            pim=shine(prod,(f-48)/30.0) if (sh and 48<=f<=78) else prod
            x=int(cx-prod.width//2); y=int(base_y-prod.height+yoff)
            sh_layer=shadow(cx,int(base_y),prod.width); paste_a(img,sh_layer,(0,0),a*0.9)
            paste_a(img,pim,(x,y),a)
        paste_a(img,ctaL,(0,int((1-ease((f-54)/16))*150)),ease((f-54)/16))
        paste_a(img,subL,(0,0),ease((f-66)/14))
        frames.append(np.asarray(img.convert("RGB")))
    imageio.mimwrite(out,frames,fps=FPS,codec="libx264",quality=8,macro_block_size=8)
    print("lagret",out,len(frames),"frames")

bottle=prep("fanta_bottle.webp",700,-5)
can=prep("fanta_can.jpg",540,7)
build([bottle,can],"NYE DRIKKER","reel_drikke.mp4")
