"""Nordic Engros – sjokolade-REEL: diagonal layout + animert opprinnelse-flagg."""
...
def flag_de(h):
    w=int(h*1.5); f=Image.new("RGBA",(w,h),(0,0,0,255)); d=ImageDraw.Draw(f)
    d.rectangle([0,0,w,h//3],fill=(20,20,20,255)); d.rectangle([0,h//3,w,2*h//3],fill=(221,0,0,255))
...
def flag_pl(h):
    ...
    m=Image.new("L",(w,h),0); ImageDraw.Draw(m).rounded_rectangle([0,0,w,h],radius=5,fill=255); f.putalpha(m); return f
def chip_layer(text):
    fh=40; flag=flag_pl(fh); tf=font(800,30); pad=24
...
def build(specs, heading, out, chip=None, subline="Nye varer på lager hver uke"):
    base=Image.new("RGBA",(W,H),CREAM+(255,)); d=ImageDraw.Draw(base)
    d.ellipse([215,855,875,1505],fill=PEACH); d.ellipse([295,930,795,1430],fill=ORANGE)
    ...
    ds.text((W//2-ds.textlength(subline,font=font(600,30))/2,1784),subline,font=font(600,30),fill=MUTE)
    chipL=chip_layer(chip[0]) if chip else None
    frames=[]
    for f in range(N):
        img=base.copy()
        paste_a(img,logoL,(0,0),ease(f/12))
        paste_a(img,headL,(0,0),ease((f-6)/14))
        for prod,cx,base_y,st,ph,sh,z in specs:
            t=(f-st)/24.0; e=ease(t); a=e
            if a<=0: continue
            entr=(1-e)*220; fl=7*math.sin(2*math.pi*(f-st)/64.0+ph) if e>=1 else 0
            yoff=entr-fl
            pim=shine(prod,(f-50)/30.0) if (sh and 50<=f<=80) else prod
            x=int(cx-prod.width//2); y=int(base_y-prod.height+yoff)
            shl=shadow(cx,int(base_y),prod.width); paste_a(img,shl,(0,0),a*0.9)
            paste_a(img,pim,(x,y),a)
        if chipL is not None:
            cs=chip[3]; t=(f-cs)/16.0
            if t>0:
                s=max(0.05,popscale(t)); cw=int(chipL.width*s); ch=int(chipL.height*s)
                cc=chipL.resize((cw,ch)); paste_a(img,cc,(chip[1]-cw//2,chip[2]-ch//2),ease((f-cs)/10.0))
    ...
    imageio.mimwrite(out,frames,fps=FPS,codec="libx264",quality=8,macro_block_size=8)
    print("lagret",out)


# --- override: TILBAKE PÅ LAGER-reel (Twaróg + Hajdú) ---
if __name__=="__main__":
    twarog=prep("lowicz_cut.png",470,-6)
    hajdu=prep("hajdu_cut.png",450,9)
    specs=[(twarog,460,1300,12,0.0,True,0),(hajdu,665,1360,24,math.pi,False,1)]
    build(specs,"TILBAKE PÅ LAGER","reel_restock.mp4",
          chip=("FRA POLEN",300,720,44),subline="Fersk vare – bestill før den er borte")
